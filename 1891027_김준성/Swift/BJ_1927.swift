...
struct Heap<T : Comparable> {
    ...
    mutating func insert(_ element : T) {
        ...
        while (index != 1) && sortFunction(elements[index], elements[parent(of: index)]) {//부모와 자식을 비교
    ...
    mutating func moveDown(from index : Int) {
        ...
        if leftChildIndex < elements.endIndex && sortFunction(elements[leftChildIndex], elements[childIndex]) {//형제노드 끼리 비교
